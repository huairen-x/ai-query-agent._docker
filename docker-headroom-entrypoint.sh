...
# Start mock upstream in background
...
echo "[Headroom] Dashboard: http://localhost:${PROXY_PORT}/dashboard"
headroom proxy --port "${PROXY_PORT}" --openai-api-url "${UPSTREAM_URL}" &
PROXY_PID=$!
sleep 3

# Start headroom dashboard (same port)
echo "[Headroom] Starting Headroom Dashboard..."
headroom dashboard --port "${PROXY_PORT}" --no-open &
DASHBOARD_PID=$!
